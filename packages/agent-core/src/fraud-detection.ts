...
import { redisGet, redisSetEx, redisSet, redisDel } from "./redis-helpers";
...
const ANOMALY_FREEZE_THRESHOLD = 75;

// ============================================
// Redis Keys
// ============================================

const HEALTH_KEY = (agentId: string, userId: string) =>
  `fraud:health:${agentId}:${userId}`;
const PATTERN_KEY = (userId: string, window: string) =>
  `fraud:pattern:${userId}:${window}`;
const ALERT_KEY = (id: string) => `fraud:alert:${id}`;
const MULTISIG_KEY = (txId: string) => `fraud:multisig:${txId}`;
const FROZEN_KEY = (agentId: string, userId: string) =>
  `fraud:frozen:${agentId}:${userId}`;
...
export async function analyzeTransactionPattern(
  ...
  if (avgAmount > 0n) {
    const deviation =
      amount > avgAmount ? amount - avgAmount : avgAmount - amount;
...
export async function updateAnomalyScore(
  ...
  health.anomalyScore = Math.max(
    0,
    Math.min(100, health.anomalyScore + scoreChange),
  );

  if (
    health.anomalyScore >= ANOMALY_FREEZE_THRESHOLD &&
    ...
    await freezeAgent(
      agentId,
      userId,
      `High anomaly score: ${health.anomalyScore}`,
    );
    await createAlert({
      userId,
      agentId,
...
export async function unfreezeAgent(
  ...
  logger.info("Agent unfrozen", { component: "fraud-detection", agentId, userId });
}
...
export async function createAlert(params: {
  ...
  logger.warn("Fraud alert created", { component: "fraud-detection", ...alert });
