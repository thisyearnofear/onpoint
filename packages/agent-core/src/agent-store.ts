...
import type { CommissionRecord } from "./commissions";
...
export async function persistAutonomyThreshold(
  ...
  await redisSet(
    KEYS.autonomyThreshold(agentId, userId),
    threshold.toString(),
  );
}
...
export async function persistSuggestion(
  ...
  const ttlSeconds = Math.ceil(
    (suggestion.expiresAt - Date.now()) / 1000 + 3600,
  );
  await Promise.all([
...
export async function persistApproval(
  ...
  const ttlSeconds = Math.ceil(
    (approval.expiresAt - Date.now()) / 1000 + 3600,
  );
  await Promise.all([
...
export async function persistCommission(
  ...
  await Promise.all([
    redisSetEx(KEYS.commission(record.id), record, 86400 * 90),
