 ...
 *
 * Ported from apps/web/lib/services/self-protocol.ts into @repo/agent-core
 * so both the API (Express) and web (Next.js) surfaces share one source of
 * truth for the unified agent identity.
 */

import { logger } from "./logger";
import { getAgentIdentity } from "./agent-registry";
 ...
 *
 * ERC-8004 fields are sourced from getAgentIdentity() (the canonical
 * ONPOINT_AGENT constant in agent-registry.ts) so the tx hash and
 * registry address always match the real on-chain registration.
 */
export async function getUnifiedAgentIdentity(
  agentWalletAddress?: string,
  erc8004AgentId?: number,
): Promise<UnifiedAgentIdentity> {
  // Pull the canonical ERC-8004 identity from agent-registry so the
  // tx hash and registry address are always the real on-chain values.
  const erc8004Identity = await getAgentIdentity().catch(() => null);

  const walletAddress = agentWalletAddress || erc8004Identity?.walletAddress || "";
  const agentId = erc8004AgentId ?? erc8004Identity?.agentId ?? 9177;

  const selfReg = await registerSelfAgent(walletAddress, agentId);
  ...
  return {
    erc8004: {
      ...
      name: "OnPoint AI Stylist",
      walletAddress,
      registryAddress: erc8004Identity?.registryAddress || "0x8004A169FB4a3325136EB29fA0ceB6D2e539a432",
