# ADR 0004 — Bright Data Web Intelligence Integration
...
3. **No changes to `product-catalog.ts`** — The TypeScript catalog service talks to the bridge via `/v1/agent/search` and receives the same `ItemData` shape. Bright Data is invisible above the bridge.

### What we do NOT build

- No new endpoints or API surfaces
- No changes to the Curator schema or storefront
- No dependency on Bright Data at the TypeScript/frontend layer
...
2. **Same interface, same output shape.** `BrightDataResult` maps to `ItemData` exactly like `TinyFishResult` does. No downstream changes needed.
...
4. **No new dependencies in the TypeScript layer.** Bright Data lives entirely in the Python bridge. The Next.js app is unaffected.

### Alignment with Core Principles

| Principle | How this applies |
|-----------|-----------------|
| **ENHANCEMENT FIRST** | Enhances existing tier chain, no new surface area |
...
| **PREVENT BLOAT** | One new client file, one import in main.py, one env var. That's it. |
| **DRY** | Follows exact same dataclass + async client pattern as `tinyfish_client.py` |
| **CLEAN** | Bright Data is behind a clean interface; the bridge doesn't know or care which provider answered |
| **MODULAR** | Self-contained `brightdata_client.py` can be tested independently |
| **PERFORMANT** | SERP API returns structured data in <1s; no browser startup overhead |
...
### Neutral
- No changes to the TypeScript layer, Curator schema, or storefront
