...
export class TokenVaultService {
  ...
  static async executeSecureRequest(
    ...
    if (!auth.permissions.includes('agent:act')) {
      throw new Error("Unauthorized: Agent lacks 'agent:act' permission");
    }
    ...
    try {
      ...
      console.log(`[TokenVault] Calling ${request.connection} ${request.method} ${request.endpoint}`);
  ...
  private static async getConnectionToken(
    ...
    try {
      ...
      console.error(`[TokenVault] Failed to get token for ${connection}:`, error.message);
   ...
   */
  static async getConnectedAccounts(auth0Id: string): Promise<ConnectedAccount[]> {
    try {
