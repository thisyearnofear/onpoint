    ...
    try {
      ...
      if (!description) {
        return NextResponse.json(
          { error: "Color palette description is required" },
...
function parseColorPaletteResponse(
  aiResponse: string,
  description: string,
  style: string,
  season: string,
  ...
  const colorNameMatches =
    aiResponse.match(/([A-Z][a-z]+\s+[A-Z][a-z]+|[A-Z][a-z]+)/g) || [];
  const colorNames = colorNameMatches
    .filter(
      (name) =>
  ...
  const maxColors = Math.min(
    7,
    Math.max(hexMatches.length, colorNames.length, 5),
  );
  ...
  for (let i = 0; i < maxColors; i++) {
    colors.push({
      ...
      description: extractColorDescription(
        aiResponse,
        colorNames[i] || fallbackColors[i]?.name || "",
      ),
    });
  }
...
function generateFallbackPalette(style: string, season: string) {
  ...
  return (
    palettes.modern[seasonKey as keyof typeof palettes.modern] ||
    palettes.modern.spring
  );
}
...
function extractColorDescription(text: string, colorName: string): string {
  ...
  for (const line of lines) {
    if (
      line.toLowerCase().includes(colorName.toLowerCase()) &&
      line.length > colorName.length + 10
    ) {
      return line.trim();
    }
  }
