...
function parseDesignResponse(aiResponse: string, originalPrompt: string) {
  ...
  const colorMatches =
    aiResponse.match(/#[0-9A-Fa-f]{6}|#[0-9A-Fa-f]{3}/g) || [];
  const colorWords = [
    "red",
    "blue",
    "green",
  ...
  const foundColors = colorWords.filter((color) =>
    aiResponse.toLowerCase().includes(color),
  );
  ...
  const materials = materialWords.filter((material) =>
    aiResponse.toLowerCase().includes(material),
  );
