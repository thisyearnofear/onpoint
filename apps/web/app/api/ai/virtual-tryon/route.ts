...
function extractStructuredStylingTips(text: string): {
    ...
    try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0 && parsed[0]?.text) {
            const valid = parsed.slice(0, 4).filter((item: any) => typeof item.text === 'string');
            return {
...
function extractOutfitRecommendations(text: string): Array<{ item: string; reason: string; priority: number }> {
    ...
    for (let i = 0; i < lines.length && recommendations.length < 5; i++) {
        ...
        if (line && (line.includes('recommend') || line.includes('suggest') || line.includes('add') || line.includes('try'))) {
            const cleaned = line.replace(/^\d+\.?\s*/, '').trim();
            if (cleaned.length > 10) {
                recommendations.push({ item: cleaned, reason: 'Based on style analysis', priority: recommendations.length + 1 });
            }
        }
    }
...
function parseVirtualTryOnResponse(aiResponse: string, type: string, originalData: { items?: Array<{ name: string; description?: string }> }) {
    if (type === 'body-analysis') {
        ...
        return { stylingTips, recommendations, analysis: aiResponse, enhancedOutfit: originalData.items || [] };
    }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!type) return NextResponse.json({ error: 'Analysis type is required' }, { status: 400, headers: corsHeaders(origin) });
        ...
        if (type === 'generate-outfit-image') {
            ...
            if (!veniceApiKey) return NextResponse.json({ error: 'Venice API key not configured' }, { status: 500, headers: corsHeaders(origin) });

            const personDescription = data.personDescription || '';
            const outfitDescription = data.items?.map((item: any) => `${item.name}: ${item.description || ''}`).join(', ');
            ...
            const veniceResponse = await fetch("https://api.venice.ai/api/v1/image/generate", {
                ...
                body: JSON.stringify({
                    ...
                    width: 512, height: 768, format: "webp"
                }),
            });

            if (!veniceResponse.ok) throw new Error(`Venice API error: ${veniceResponse.status}`);
        ...
        const { text, usedProvider } = await generateText({
            ...
            provider,
            geminiModel: modelChoice === 'pro' ? 'gemini-1.5-pro' : 'gemini-1.5-flash',
            openaiModel: modelChoice === 'pro' ? 'gpt-4o' : 'gpt-4o-mini'
        });

        const analysisData = parseVirtualTryOnResponse(text || '', type, data);
        return NextResponse.json({ ...analysisData, provider: usedProvider, type }, { headers: corsHeaders(origin) });
