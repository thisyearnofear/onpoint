    ...
    if (!result.success) {
      return NextResponse.json(
        { 
          ...
          connectionUrl: '/api/auth/connect?connection=google-oauth2&scope=https://www.googleapis.com/auth/calendar.events'
        },
        { status: 403 }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await CalendarTool.getUpcomingEvents(authResult.context, daysAhead);
