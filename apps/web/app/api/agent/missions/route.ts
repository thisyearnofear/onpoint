...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId || !eventType) {
      return NextResponse.json(
        { success: false, error: "Missing required fields: userId, eventType" },
        { status: 400 }
      );
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!userId || !missionId) {
      return NextResponse.json(
        { success: false, error: "Missing required fields: userId, missionId" },
        { status: 400 }
      );
    }
    ...
    if (!result.success) {
      return NextResponse.json(
        { success: false, error: "Cannot claim reward — mission not completed or already claimed" },
        { status: 400 }
      );
    }
