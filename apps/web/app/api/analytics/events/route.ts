...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const metrics = {
      provider_selections: events.filter((e) => e.event === "provider_selected")
        .length,
      sessions_started: events.filter((e) => e.event === "live_session_started")
        .length,
      sessions_ended: events.filter((e) => e.event === "live_session_ended")
        .length,
      payments_initiated: events.filter((e) => e.event === "payment_initiated")
        .length,
      payments_completed: events.filter((e) => e.event === "payment_completed")
        .length,
    };
